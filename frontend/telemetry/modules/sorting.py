...
def section_List(data_List, boat_Data, return_length=False):
    ...
    if len(data_List) > 0:
        ...
        for idx, dist in enumerate(stroke_Distances):
            ...
            stroke_data = [x for x in stroke_data if x is not None]
...
def section_Data(data_List, boat_Data, get_Size=False):
    ...
    # Check if List is not empty (when seat sensors are not plugged in).
    if not [] in sectioned_List:
        for section in sectioned_List:
    ...
    for array_Container in data_List:
        ...
        for i in range(0,len(array_Container[0])):
            ...
            for sectioned_Array in array_Container:
                try:
                    if sectioned_Array[i] != None:
                        numbers_of_scanned_index.append(sectioned_Array[i])
                    else:
                        continue
                except: pass
            if len(numbers_of_scanned_index) > 0:
